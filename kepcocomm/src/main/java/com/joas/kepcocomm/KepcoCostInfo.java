...
public class KepcoCostInfo {
    public static final int KEPCO_COST_TIME_INFO_CNT = 48;
    public static final int KEPCO_LOAD_CL_MIN = 1; // 경부하
    public static final int KEPCO_LOAD_CL_MID = 2; // 중부하
    public static final int KEPCO_LOAD_CL_MAX = 3; // 최대부하

    public String version = "";       //단가버젼
    //public string last_membdate;

    public double[] KWHBILL_UCOST = new double[KEPCO_COST_TIME_INFO_CNT];
    public double[] INFRABILL_UCOST = new double[KEPCO_COST_TIME_INFO_CNT];
    public double[] SVC_BILL_UCOST = new double[KEPCO_COST_TIME_INFO_CNT];
    public int[] LOAD_CL = new int[KEPCO_COST_TIME_INFO_CNT];

    public KepcoCostInfo() {
        for (int i = 0; i < KEPCO_COST_TIME_INFO_CNT; i++) {
            KWHBILL_UCOST[i] = 173.8; // 혹시 잘못된 경우 0원으로 계산되지 않기 위함
            INFRABILL_UCOST[i] = 0.0;
            SVC_BILL_UCOST[i] = 0.0;
            LOAD_CL[i] = KEPCO_LOAD_CL_MID;
        }
    }
