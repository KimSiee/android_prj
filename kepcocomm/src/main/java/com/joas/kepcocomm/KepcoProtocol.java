...
public class KepcoProtocol {
    ...
    public enum KepcoCmd {
        ...
        GLOB_LOCK_OPEN_30(0x30),//원격도어 오픈 요청(원격인증)
        ...
        public static KepcoCmd getValue(int _id) {
            KepcoCmd[] As = KepcoCmd.values();
            for (int i = 0; i < As.length; i++) {
                if (As[i].Compare(_id))
                    return As[i];
            }
            return KepcoCmd.KEPCO_CMD_END;
        }

        public static KepcoCmd getValue(String name) {
            KepcoCmd[] As = KepcoCmd.values();
            for (int i = 0; i < As.length; i++) {
    ...
    public enum KepcoRet {
        ...
        PKT_YES(0x59), //  성공 (OK)
        KEPCO_CMDACK_END(0xFF);

        int id;

        private KepcoRet(int id) {
            this.id = id;
        }
        ...
        public static KepcoRet getValue(int _id) {
            KepcoRet[] As = KepcoRet.values();
            for (int i = 0; i < As.length; i++) {
                if (As[i].Compare(_id))
                    return As[i];
            }
            return KepcoRet.KEPCO_CMDACK_END;
        }
    }
