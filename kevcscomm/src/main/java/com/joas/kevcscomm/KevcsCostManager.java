...
public class KevcsCostManager {
    ...
    private KevcsCostInfo currentCostInfo;
    ...
    double noMemberCost = 430;  // 기본 단가 확인하기

    int base_paym_amt = 5000;
    ...
    public KevcsCostManager(String filePath) {
    ...
    public String getLastVersion() {
        String ver = "";
        for (int i = 0; i < CostTable.KEVCS_COST_MAX_CNT; i++) {
    ...
    public KevcsCostInfo getCostInfoDate(String date) {
        KevcsCostInfo cInfo = null;
    ...
    public KevcsCostInfo getCurrentCostInfo() {
    ...
    public void saveKevcsCostTable(String version, double[] kwhbill_ucost, double[] infrabill_ucost, double[] svc_bill_ucost, int[] load_cl) {
        ...
        if (findIdx == -1)
        {
            for (int i = 0; i < CostTable.KEVCS_COST_MAX_CNT - 1; i++)
            {
                costTable.list[i] = costTable.list[i + 1];
            }
            findIdx = CostTable.KEVCS_COST_MAX_CNT - 1;

            // 비어있는 슬롯 새로 생성
            costTable.list[findIdx] = new KevcsCostInfo();
        }

        //3. 값을 채운다.
        costTable.list[findIdx].version = version;
        System.arraycopy(kwhbill_ucost, 0, costTable.list[findIdx].KWHBILL_UCOST, 0, KevcsCostInfo.KEVCS_COST_TIME_INFO_CNT);
        System.arraycopy(infrabill_ucost, 0, costTable.list[findIdx].INFRABILL_UCOST, 0, KevcsCostInfo.KEVCS_COST_TIME_INFO_CNT);
        System.arraycopy(svc_bill_ucost, 0, costTable.list[findIdx].SVC_BILL_UCOST, 0, KevcsCostInfo.KEVCS_COST_TIME_INFO_CNT);
        System.arraycopy(load_cl, 0, costTable.list[findIdx].LOAD_CL, 0, KevcsCostInfo.KEVCS_COST_TIME_INFO_CNT);
    ...
    // 충전 시작시 불려짐
    public void startCostCalc(double meter_val, int ucost_aply_tp, int tax_tp, int time_cost ,boolean isAuthCredit)
    {
        initCostSumValues();
        updateCurrentCostTable();
        ...
        {
            startTimeCost = getCurTimeCostVal(time_cost, isAuthCredit);
            ...
            if(isAuthCredit) {  //비회원일때
                startKwhCost = noMemberCost;
            }else{
    ...
    public double getCurTimeCostVal(int time_cost, boolean isAuthCredit)
    {
        if(isAuthCredit){   //비회원 INFRABILL_UCOST 를 72인가? 에서 받는걸로 변경 해준다. swpark 201120
            return noMemberCost;
        }
        else{   //회원
    ...
    public void processCostCalc(double meter_val, int time_cost, boolean isAuthCredit, boolean isAuthRemote)
    {
        // 차이 계산
        ...
        if (aplyMode == APLY_MODE_START)
        {
            if(isAuthCredit || isAuthRemote) curCost = noMemberCost;
            else curCost = startTimeCost;
        ...
        {
            curCost = getCurTimeCostVal(time_cost, isAuthCredit);
    ...
    public ChargingPeriodInfo getAndResetPeriodData(int time_cost, boolean isAuthCredit) {
        ...
        synchronized (periodInfoLock) {
            ...
            if (aplyMode == APLY_MODE_START) {
                ...
                periodInfo.periodUCost = getCurTimeCostVal(time_cost, isAuthCredit);
            }
    ...
    {
        public static final int KEVCS_COST_MAX_CNT = 3;

        public KevcsCostInfo[] list = new KevcsCostInfo[KEVCS_COST_MAX_CNT];

        public CostTable() {
            for (int i = 0; i < KEVCS_COST_MAX_CNT; i++)
                list[i] = new KevcsCostInfo();
        }
    ...
    public double getNoMemberCost(){
        return noMemberCost;
    }
    public int getBasePaymAmt() { return base_paym_amt; }
}
