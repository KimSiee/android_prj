...
public class KevcsPacket {
    ...
    // 새로운 패킷을 생성
    public KevcsPacket(int chargerID, byte[] ver, byte[] dst, byte[] src, int seq) {
        initHeader(chargerID, ver, dst, src, seq);
    }

    public KevcsPacket(byte[] buffer, int size) {
    ...
    public void parsePacket(byte[] buffer, int size) {
        try {
            ...
            int idx = KevcsProtocol.HEADER_MAGIC.length;
            ...
            LogWrapper.e("KevcsPacket", "parsePacket():"+e.toString());
        }
    }
    ...
    public void encodeAck(int cmd, int ret, String[] listData, KevcsPacket recvPacket) {
    ...
    public void encode(int cmd, int ret, String[] listData) {
        try {
            ...
            int packetSize = KevcsProtocol.HEADER_SIZE + 7;
            if (vdDataRaw != null) {
            ...
            // MAGIC(EMBLEM)
            int idx = 0;
            System.arraycopy(KevcsProtocol.HEADER_MAGIC, 0, raw, idx, KevcsProtocol.HEADER_MAGIC.length);
            idx += KevcsProtocol.HEADER_MAGIC.length;
            ...
            LogWrapper.e("KevcsPacket", "encode():"+e.toString());
        }
    }
    ...
    public String toString() {
        String str ="";
        try {
            str =   "cmd:0x" + Integer.toHexString(cmd) + "("+ KevcsProtocol.KevcsCmd.getValue(cmd).name() +"), " +
                    "ret:0x" + Integer.toHexString(ret) + "("+ KevcsProtocol.KevcsRet.getValue(ret).name() +"), " +
            ...
            String extra = "";
            if (cmd == KevcsProtocol.KevcsCmd.GLOB_FIRM_UPDATE_71.getID() &&
                    (ret == KevcsProtocol.KevcsRet.PKT_DATA.getID() || ret == KevcsProtocol.KevcsRet.PKT_COMPT.getID())) {
                vdLength = dataList[0].length() + dataList[1].length() + dataList[2].length() + 3;
                extra = "binary skip..";
            } /*else if (cmd == KevcsProtocol.KevcsCmd.GLOB_QRIMG_DOWN_73.getID() &&
                    (ret == KevcsProtocol.KevcsRet.PKT_DATA.getID() || ret == KevcsProtocol.KevcsRet.PKT_COMPT.getID())) {
