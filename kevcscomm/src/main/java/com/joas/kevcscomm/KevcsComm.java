...
import static com.joas.kevcscomm.KevcsProtocol.KevcsCmd.*;
import static com.joas.kevcscomm.KevcsProtocol.KevcsRet.*;

public class KevcsComm extends Thread implements TL3500SListener {
    public static final String TAG = "KevcsComm";
    ...
    public static final byte[] PROTOCOL_VERSION = {(byte)0x02, (byte)0x02, (byte)0x00, (byte)0x02}; // 2.2.2
    ...
    KevcsCommListener kevcsCommListener = null;
    //KevcsCommMonitorListener joasCommMonitorListener = null;
    ...
    ConcurrentLinkedDeque<KevcsPacket> sendServerQueue = new ConcurrentLinkedDeque<KevcsPacket>();
    Thread sendMsgThread;

    byte[] recvBuf = new byte[RECV_BUFF_MAX];
    byte[] recvBufSerial = new byte[RECV_BUFF_MAX];
    KevcsPacket lastSendPacket = null;
    ...
    KevcsChargerInfo chargerInfo = new KevcsChargerInfo();
    KevcsCostManager costManager;
    ...
    KevcsCommDbHelper commDbHelper;
    KevcsMember kevcsMember;
    String basePath = "";
    ...
    // ChargeCtl
    List<KevcsChargeCtl> chargeCtlList = Collections.synchronizedList(new ArrayList<KevcsChargeCtl>());

    boolean isLocalMode = true;

    // 미전송 전문 전송
    KevcsLostTransManager lostTransManager;
    ...
    public KevcsComm(Activity activity, String firmVer, String svrip, int svrport, String cid, String authKey, String baseFilePath, String devPayTerminal, boolean usePayTerm) {
        initKevcsComm(activity, firmVer, svrip, svrport, cid, authKey, baseFilePath, devPayTerminal, usePayTerm, PayTerminalKind.STANDARD, true);
    }

    public KevcsComm(Activity activity, String firmVer, String svrip, int svrport, String cid, String authKey, String baseFilePath, String devPayTerminal, boolean usePayTerm, PayTerminalKind payTerminal, boolean useQRCode) {
        initKevcsComm(activity, firmVer, svrip, svrport, cid, authKey, baseFilePath, devPayTerminal, usePayTerm, payTerminal, useQRCode);
    }

    void initKevcsComm(Activity activity, String firmVer, String svrip, int svrport, String cid, String authKey, String baseFilePath, String devPayTerminal, boolean usePayTerm, PayTerminalKind payTerminal, boolean useQRCode) {
        ...
        costManager = new KevcsCostManager(baseFilePath);

        commDbHelper = new KevcsCommDbHelper(androidContext, baseFilePath);
        commDbHelper.open();

        kevcsMember = new KevcsMember(commDbHelper);
        lostTransManager = new KevcsLostTransManager(commDbHelper);
    ...
    void loadChargerInfo() {
        ...
        if (file.exists()) {
            try {
                ...
                chargerInfo.localMode = local_mode.equals("1") ? KevcsProtocol.LocalMode.CARDNUM_SEARCH : KevcsProtocol.LocalMode.CARD_TAG_AUTO;
            }
            catch (Exception e) {}
        }
    }
    ...
    public KevcsChargerInfo getChargerInfo() { return chargerInfo; }
    public TL3500S getTl3500s() { return tl3500s; }

    public void setListener(KevcsCommListener listener) {
        kevcsCommListener = listener;
    }
    ...
    public KevcsCostManager getCostManager() { return costManager; }
    ...
    public void run() {
        ...
        while ( !isEndFlag ) {
            try {
                ...
                if ( kevcsCommListener != null ) kevcsCommListener.onKevcsCommConnected();
                ...
                // Disconnect Event
                if ( kevcsCommListener != null ) kevcsCommListener.onKevcsCommDisconnected();
                isLocalMode = true;
    ...
    void RecvProcess(InputStream readStream, byte[] buffer) {
        ...
        while ( !isEndFlag ) {
            ...
            if (bufCnt < KevcsProtocol.HEADER_SIZE) continue;

            // 나머지 Length만큼 읽어들인다.
            int length = ByteUtil.makeInt(buffer, KevcsProtocol.HEADER_LENGTH_FIELD, true);
            int packetSize = (KevcsProtocol.HEADER_SIZE + length);
            ...
            if ( ByteUtil.arrayCompare(KevcsProtocol.HEADER_MAGIC, 0, buffer, 0, KevcsProtocol.HEADER_MAGIC.length) == false )
            {
                LogWrapper.e(TAG, "RecvProcess(): 헤더가 맞지 않음:"+ByteUtil.byteArrayToHexStringDiv(buffer, 0, KevcsProtocol.HEADER_MAGIC.length, '-'));
                bufCnt = 0;
                continue;
            }
     ...
     */
    void lastSendPacketAckProcess(KevcsPacket recvPacket) {
    ...
    void dataToLog(byte[] data, int size, String tag) {
        ...
        if ( kevcsCommListener != null ) kevcsCommListener.onKevcsCommLogEvent(tag, strLog);
    }

    void dataToLog(String tag, String data) {
        FileUtil.appendDateLog(basePath+COMM_LOG_PATH, tag+">"+ data);
        if ( kevcsCommListener != null ) kevcsCommListener.onKevcsCommLogEvent(tag, data);
    }

    synchronized void recvPacketProcess(byte[] packet, int packetSize) {
        try {
            KevcsPacket recvPacket = new KevcsPacket(packet, packetSize);
            KevcsProtocol.KevcsCmd cmd = KevcsProtocol.KevcsCmd.getValue(recvPacket.cmd);
            ...
            switch (cmd) {
                ...
                case INFO_CHARGEUCOST_REQ_22:
                    ...
                    break;
                case GLOB_QRCODE_CERT_33:
                    onGlobLockOpen_33(recvPacket);
                    break;
                case EV_CHARGE_STOP_39:
                    onGlobChargeStop_39(recvPacket);
                    break;
    ...
    synchronized void sendPacket(KevcsPacket packet) {
        try {
    ...
    synchronized void sendPayTerminalPacket(KevcsPacket packet) {
        if (usePayTerminal && payTerminalKind == PayTerminalKind.STANDARD) {
            ...
            LogWrapper.d(TAG, "No use payTerminal!! pkt:" + KevcsProtocol.KevcsCmd.getValue(packet.cmd).name());
        }
    }
    ...
    void timerTaskPeriodSecond() {
        ...
        if ( timerPingCnt++ > KevcsProtocol.PING_MSG_PERIOD ) {
        ...
        if ( usePayTerminal ) {
            if ( payTerminalKind == PayTerminalKind.STANDARD ) {
                if (timerPaymStatCnt++ > KevcsProtocol.PING_MSG_PERIOD) {
                    sendGlobPaymStat_61();
                    timerPaymStatCnt = 0;
                }
                ...
                if (timerPaymStatAckCnt > 0) {
                    ...
                    if (timerPaymStatAckCnt == 0) {
                        ...
                        if (timerPaymStatErrCnt > 3) {
                            if (kevcsCommListener != null) kevcsCommListener.onPaymStatErr(true);
                            isPaymStatErr = true;
                ...
                if (timerPaymIntyCnt++ > KevcsProtocol.PAYM_INTY_PERIOD) {
    ...
    void processChargingInfoPeriod(int time_cost) {
        chargerInfo.cp_stat = KevcsProtocol.CPStat.CHARGING;
        KevcsCostManager.ChargingPeriodInfo info = costManager.getAndResetPeriodData(time_cost, chargerInfo.pay_mthd == 1);
    ...
    void updateLoadClAndUCost(int time_cost) {
        ...
        chargerInfo.current_ucost = costManager.getCurTimeCostVal(time_cost,chargerInfo.pay_mthd == 1);
    }
    ...
    public double getCurrentCostUnit() {
        ...
        return costManager.getCurTimeCostVal(timeCost, chargerInfo.pay_mthd == 1);
    }

    void processChargeCtl() {
        Iterator<KevcsChargeCtl> it = chargeCtlList.listIterator();
        while (it.hasNext()) {
            KevcsChargeCtl chargeCtl = it.next();

            switch (chargeCtl.state) {
                case NONE:
                    if ( chargeCtl.isStartCondition(isChargingState) ) {
                        chargeCtl.state = KevcsChargeCtl.State.READY;
                        ...
                        else if ( kevcsCommListener != null ) kevcsCommListener.onChargeCtl(chargeCtl);
                    }
                    break;
    ...
    void onConnectedProcess() {
        ...
        if ( onBootMsgSent == false ) {
            ...
            if ( kevcsCommListener != null ) kevcsCommListener.onKevcsCommConnectedFirst();
        }
        if ( isChargingState == false ) {
            sendGlobStatPing_10();
        }
    ...
    void initStatVal() {
        ...
        chargerInfo.charge_req_cfm_mthd = KevcsProtocol.CHARGE_REQ_CFM_METHOD_FULL; // 1:full, 2:전력량, 3:금액
        chargerInfo.charge_req_kwh = 99.9;
        chargerInfo.charge_req_amt = 0;
        chargerInfo.pay_mthd = KevcsProtocol.PAY_METHOD_MEMBER;
        ...
        chargerInfo.charge_end_stat = KevcsProtocol.CHARGE_END_FULL; // Full 충전

        chargerInfo.isPaymentCharging = false;
        chargerInfo.initKevcsPayRet();
    }

    public void setCPStat(KevcsProtocol.CPStat state) {
        ...
        switch ( chargerInfo.cp_stat) {
            case READY:
                ...
                if (chargerInfo.isPaymentCharging) {
                    ...
                    if ( chargerInfo.charge_end_stat == KevcsProtocol.CHARGE_END_FULL ) {
                        chargerInfo.charge_end_stat = KevcsProtocol.CHARGE_END_FAULT;
                    }
                    //sendEvChargeEnd_35(); 비회원 결제시 오류 발생으로 인해 삭제 swpark 210317 V1.2
                }
    ...
    public void setOutletType(KevcsProtocol.OutletType type) {
    ...
    public void startCharge(double meterVal) {
        ...
        Calendar c = Calendar.getInstance();
        int curTimeSec = (c.get(Calendar.HOUR_OF_DAY)*3600)+(c.get(Calendar.MINUTE)*60) + c.get(Calendar.SECOND);
        costManager.startCostCalc(meterVal, chargerInfo.rxInfo.ucost_aply_tp, chargerInfo.rxInfo.tax_tp,  curTimeSec/1800, chargerInfo.pay_mthd == 1);
        ...
        setCPStat(KevcsProtocol.CPStat.CHARGING);
    }
    ...
    public void finishCharge() {
        ...
        processChargingInfoPeriod(curTimeSec/1800);
        setCPStat(KevcsProtocol.CPStat.FINISH_CHARGE);

        sendEvChargeEnd_35();

        processChargingInfoPeriod(curTimeSec/1800);
        setCPStat(KevcsProtocol.CPStat.FINISH_CONNECTING);
    }

    void localAuthProcess() {
        boolean isSuccess = false;
        // 카드테깅이고 자동인증일 경우 무조건 승인
        if ( chargerInfo.localMode == KevcsProtocol.LocalMode.CARD_TAG_AUTO && chargerInfo.cert_tp == 1) {
            isSuccess = true;
        }
        else if ( chargerInfo.localMode == KevcsProtocol.LocalMode.CARDNUM_SEARCH ) {

            if ( chargerInfo.cert_tp == 1 || chargerInfo.cert_pass.length() == 0) { // 카드테깅 혹은 번호조회일경우
                isSuccess = kevcsMember.searchMember(chargerInfo.member_card_no);
            }
            else { // 번호입력일경우 패스워드검사
                isSuccess = kevcsMember.searchMember(chargerInfo.member_card_no, chargerInfo.cert_pass);
            }
        }

        if ( kevcsCommListener != null ) kevcsCommListener.onKevcsCommAuthResult(isSuccess);
    }
    ...
    void onErrorChangeLocalMode() {
        ...
        if ( kevcsCommListener != null ) kevcsCommListener.onCommLocalMode(true);
        isLocalMode = true;
    }
    ...
    void processRetransLostPacket() {
        int addCount = 0;
        try {
            KevcsPacket[] list = lostTransManager.getListPacket();
            if (list != null) {
                //Queue에서 해당 msgId가 없을때 add한다.
                for (int i = 0; i < list.length; i++) {
                    boolean isContain = false;
                    Iterator<KevcsPacket> itor = sendServerQueue.iterator();
                    while (itor.hasNext()) {
                        KevcsPacket packet = itor.next();
                        //packet.messageID
    ...
    public void payRequest(int payType, double kwh, int amt) {
        ...
        chargerInfo.pay_mthd = KevcsProtocol.PAY_METHOD_CREDIT; // 신용카드결제

        if ( payTerminalKind == PayTerminalKind.STANDARD) sendGlobChargeAmt_62();
        else if ( payTerminalKind == PayTerminalKind.TL3500S ){
            tl3500s.payReq_G(amt, 0, true, 0);
            // 카드가 꽂아져 있는 상태에서 인증 진행시 처리
            if ( tl3500CardInputState.equals("I")) kevcsCommListener.onKevcsTL3500CardEvent("I");
        }
    }

    public void payRealRequest(int payType, double kwh, int amt) {
        chargerInfo.paym_aprv_type = ""+ payType;
        chargerInfo.paym_charge_kwh = String.format("%.2f", kwh );
        chargerInfo.paym_charge_amt = ""+ amt;
        ...
        if ( payTerminalKind == PayTerminalKind.STANDARD) sendGlobChargeAmt_62();
            ...
            tl3500s.payReq_G((int)amt, 0, false, 0);
            // 카드가 꽂아져 있는 상태에서 인증 진행시 처리
            if ( tl3500CardInputState.equals("I")) kevcsCommListener.onKevcsTL3500CardEvent("I");
        }
    }
    ...
    public void payTerminalResetReq() {
        if ( payTerminalKind == PayTerminalKind.STANDARD ) {
            KevcsChargeCtl chargeCtl = new KevcsChargeCtl();
    ...
    KevcsPacket getNewSendRequestPacket() {
        return new KevcsPacket(chargerID, PROTOCOL_VERSION, rawServerIP, rawLocalIP, getNewSeqNumber());
    }

    public void sendGlobInitStart_11() {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();

        Vector<String> data = new Vector<>();
        data.add(serverAuthKey);
        data.add("0"); // 수신 Port 사용안함

        kevcsPacket.encode(GLOB_INIT_START_11.getID(), PKT_REQUEST.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    public void sendGlobInitEnd_12() {
        ...
        kevcsPacket.encode(GLOB_INIT_END_12.getID(), PKT_REQUEST.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendGlobStatPing_10() {
        ...
        kevcsPacket.encode(GLOB_STAT_PING_10.getID(), PKT_REQUEST.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendPaymStatInfo_13() {
        ...
        data.add("V0"); //한충전 테크리더는 강제로 V펌웨어 버전을 보내준다.   swpark 201125
        ...
        kevcsPacket.encode(PAYM_STAT_INFO_13.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }


    void sendGlobChargeCtl_15(KevcsProtocol.KevcsRet ret) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();

        Vector<String> data = new Vector<>();
        //swpark 201109 한충전용 제거
        //data.add(""+chargerInfo.outlet_id);

        kevcsPacket.encode(GLOB_CHARGE_CTL_15.getID(), ret.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }

    public void sendGlobChargeCtlBegin_15(KevcsChargeCtl ctl) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        ...
        kevcsPacket.encode(GLOB_CHARGE_CTL_15.getID(), PKT_BEGIN.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }

    public void sendGlobChargeCtlCompt_15(KevcsChargeCtl ctl, boolean isSuccess) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        ...
        kevcsPacket.encode(GLOB_CHARGE_CTL_15.getID(), PKT_COMPT.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    public void sendGlobFaultEvent_16(FaultInfo faultInfo, int stat) {
        ...
        kevcsPacket.encode(GLOB_FAULT_EVENT_16.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendInfoChargeUCostReq_22() {
        ...
        kevcsPacket.encode(INFO_CHARGEUCOST_REQ_22.getID(), PKT_REQUEST.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }

    void sendEvChargeStop_39(String reqId, String outletId, String membercard, String resultCd, KevcsPacket recvPacket) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        ...
        kevcsPacket.encodeAck(EV_CHARGE_STOP_39.getID(), PKT_YES.getID(), data.toArray(new String[data.size()]), recvPacket);
        AddSendServerPacket(kevcsPacket);
    }

    void sendGlobLockOpenAck_33(String reqId, String membercard, String outletId, String chg_req_amt, String connType, String resultCd, String resultMsg, KevcsPacket recvPacket) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        ...
        data.add(membercard);
        data.add(outletId);
        data.add(chg_req_amt);
        data.add(connType);
        data.add(resultCd);
        data.add(resultMsg);

        kevcsPacket.encodeAck(GLOB_QRCODE_CERT_33.getID(), PKT_YES.getID(), data.toArray(new String[data.size()]), recvPacket);
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendEvMembCert_31() {
        ...
        kevcsPacket.encode(EV_MEMB_CERT_31.getID(), PKT_REQUEST.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendEvChargeStat_34() {
        ...
        //swpark 201109 한충전용 최대크기 5로 변경
        data.add(String.format("%.4f", costManager.getSumChargeKwh(KevcsCostManager.IDX_SUM_TOTAL)));   //최대크기 5로 변경
        data.add(String.format("%.4f", costManager.getSumChargeCost(KevcsCostManager.IDX_SUM_TOTAL)));
        ...
        kevcsPacket.encode(EV_CHARGE_STAT_34.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendEvChargeEnd_35() {
        ...
        data.add(String.format("%.4f", costManager.getSumChargeKwh(KevcsCostManager.IDX_SUM_TOTAL))); // 충전량 합계
        ...
        //swpark 201109 한충전용 프로토콜 수정
       /* data.add(String.format("%.4f", costManager.getSumChargeKwh(KevcsCostManager.IDX_SUM_MIN))); // 경부하 충전량
        data.add(String.format("%.4f", costManager.getSumChargeKwh(KevcsCostManager.IDX_SUM_MID))); // 중간부하 충전량
        data.add(String.format("%.4f", costManager.getSumChargeKwh(KevcsCostManager.IDX_SUM_MAX))); // 최대부하 충전량

        data.add(""+(int)costManager.getSumChargeCost(KevcsCostManager.IDX_SUM_MIN)); // 경부하 충전요금
        data.add(""+(int)costManager.getSumChargeCost(KevcsCostManager.IDX_SUM_MID)); // 중간부하 충전요금
        data.add(""+(int)costManager.getSumChargeCost(KevcsCostManager.IDX_SUM_MAX)); // 최대부하 충전요금

        data.add(isLocalMode ? "2" : "1"); // 서버통신모드
        data.add(""+chargerInfo.rxInfo.ucost_aply_tp);
        data.add(""+chargerInfo.rxInfo.tax_tp);*/

        kevcsPacket.encode(EV_CHARGE_END_35.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    ...
    void sendGlobCardInfo_37(KevcsProtocol.KevcsRet ret) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        Vector<String> data = new Vector<>();
        ...
        kevcsPacket.encode(GLOB_CARD_INFO_37.getID(), ret.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendGlobCardAprv_38() {
        ...
        //한충전용 프로토콜 수정 swpark 201109
        //data.add(chargerInfo.payRetInfo.trade_id);
        data.add(""+chargerInfo.outlet_id);

        kevcsPacket.encode(GLOB_CARD_APRV_38.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendGlobPaymStat_61() {
        ...
        kevcsPacket.encode(GLOB_PAYM_STAT_61.getID(), PKT_REQUEST.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    ...
    void sendGlobChargeAmt_62() {
        ...
        kevcsPacket.encode(GLOB_CHARGE_AMT_62.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendGlobPaymComm_64() {
        ...
        kevcsPacket.encode(GLOB_PAYM_COMM_64.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendGlobPaymAnt_65(boolean onoff) {
        ...
        Vector<String> data = new Vector<>();
        data.add(onoff ? "1" : "0");
        kevcsPacket.encode(GLOB_PAYM_ANT_65.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
    ...
    public void sendGlobPaymInty_66(boolean isOnBoot) {
        ...
        kevcsPacket.encode(GLOB_PAYM_INTY_66.getID(), PKT_REQUEST.getID(), data.toArray(new String[data.size()]));

        // 결제 단말기쪽으로 패킷 전송
        AddSendServerPacket(kevcsPacket);
    }

    public void sendGlobPaymCtl_68(KevcsChargeCtl chargeCtl) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        ...
        kevcsPacket.encode(GLOB_PAYM_CTRL_68.getID(), PKT_PUSH.getID(), data.toArray(new String[data.size()]));

        // 결제 단말기쪽으로 패킷 전송
        AddSendServerPacket(kevcsPacket);
    }

    void sendGlobFirmUpdate_71(KevcsProtocol.KevcsRet ret) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        Vector<String> data = new Vector<>();
        ...
        kevcsPacket.encode(GLOB_FIRM_UPDATE_71.getID(), ret.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }
    ...
    void sendGlobCpConfig_72() {
        ...
        kevcsPacket.encode(GLOB_CP_CONFIG_72.getID(), PKT_DATA.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }

    /*void sendGlobQRImgDown_73(KevcsProtocol.KevcsRet ret) {
        KevcsPacket kevcsPacket = getNewSendRequestPacket();
        ...
        kevcsPacket.encode(GLOB_QRIMG_DOWN_73.getID(), ret.getID(), data.toArray(new String[data.size()]));
        AddSendServerPacket(kevcsPacket);
    }*/
    ...
    void onGlobStatPing_10(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        KevcsProtocol.LocalMode newMode = local_mode.equals("1") ? KevcsProtocol.LocalMode.CARDNUM_SEARCH : KevcsProtocol.LocalMode.CARD_TAG_AUTO;
        ...
        if ( kevcsCommListener != null ) {
            kevcsCommListener.onTimeUpdate(timeStamp);
            //한충전 mng_pass 가 없기 때문에 없데이트 하지 않음
            //kevcsCommListener.onOnlineModeAndPwdUpdate(mng_pass);
        }
        isLocalMode = false;

        processRetransLostPacket();

        timerPaymStatCnt = KevcsProtocol.PING_MSG_PERIOD - 1; // PaymStat를 Ping 메시지 뒤 1초후 전송
    }

    void onGlobInitStart_11(KevcsPacket recvPacket) {
        int idx = 0;
        String timeStamp = recvPacket.getDataAt(idx++); // 기준시각
        String firmware = recvPacket.getDataAt(idx++); // 펌웨어 정보

        if ( kevcsCommListener != null ) kevcsCommListener.onTimeUpdate(timeStamp);
    }

    void onPaymStatInfo_13(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        try {
            ...
            if ( chargerInfo.rxInfo.paym_setup_yn.toLowerCase().equals("y") ) {
                ...
                //else if ( payTerminalKind == PayTerminalKind.TL3500S ) sendConfigTL3500();    한충전은 사용하지 않음
            }
        } catch (Exception e) {
    ...
    void onGlobChargeCtl_15(KevcsPacket recvPacket) {
        int idx = 0;
    ...
    void onChargeUCostReq_22(KevcsPacket recvPacket) {
        ...
        try {
            ...
            costManager.saveKevcsCostTable(ymd, kwhbill_ucost, infrabill_ucost, svcbill_ucost, load_cl);
        } catch (Exception e ) {
            LogWrapper.e(TAG, "onChargeUCostReq_22 e:"+e.toString());
        }
        costManager.updateCurrentCostTable();
    }
    void onGlobChargeStop_39(KevcsPacket recvPacket) {
        int idx = 0;
        String reqId = recvPacket.getDataAt(idx++);
        String outletId = recvPacket.getDataAt(idx++);
        String membercard = recvPacket.getDataAt(idx++);

        String resultCd = "1";      //디폴트 정상종료

        try {
            if (chargerInfo.cp_stat != KevcsProtocol.CPStat.CHARGING || chargerInfo.outlet_id != Integer.parseInt(outletId)) {
                //에러 : 충전상태 아님
                resultCd = "2";
            } else if (!membercard.equals(chargerInfo.member_card_no)) {
                //에러 : 충전종료 불가(카드번호 맞지않음)
                resultCd = "3";
            } else if (chargerInfo.cp_stat == KevcsProtocol.CPStat.CHARGING) {
                // 정상종료 : 충전중일 경우
                resultCd = "1";

                boolean ret = false;
                if (kevcsCommListener != null) ret = kevcsCommListener.onRemoteChargingStopReq();
                if (ret == false) {
                    resultCd = "9";
                }
            } else {
            ...
            sendEvChargeStop_39(reqId, outletId, membercard, resultCd, recvPacket);
        } catch (Exception e) {
            LogWrapper.e(TAG, "onGlobChargeStop_39 e:" + e.toString());
        }
    }

    void onGlobLockOpen_33(KevcsPacket recvPacket) {
        int idx = 0;
        String reqId = recvPacket.getDataAt(idx++);
        String membercard = recvPacket.getDataAt(idx++);
        String outletId = recvPacket.getDataAt(idx++);
        String chg_req_amt = recvPacket.getDataAt(idx++);
        String connType = recvPacket.getDataAt(idx++);

        String resultCd = "1";// 디폴트 정상
        String resultMsg = "정상LOCK 오픈";
        try {
            if (chargerInfo.cp_stat != KevcsProtocol.CPStat.READY || chargerInfo.outlet_id != Integer.parseInt(outletId)) {
                resultCd = "2";
                resultMsg = "해당 아웃렛ID 사용불가";
            } else if (chargerInfo.charge_tp == KevcsProtocol.CHARGER_TYPE_SLOW && !connType.matches("1|2")) {
                resultCd = "5";
                resultMsg = "해당 아웃렛타입 없음";

            } else if (chargerInfo.charge_tp == KevcsProtocol.CHARGER_TYPE_FAST && !connType.matches("3|4|5")) {
                resultCd = "5";
                resultMsg = "해당 아웃렛타입 없음";
            }
            else {
                chargerInfo.member_card_no = membercard;
                chargerInfo.charge_req_amt = Integer.parseInt(chg_req_amt);
                boolean ret = false;
                if ( kevcsCommListener != null ) ret = kevcsCommListener.onRemoteAuthReq(connType);
                if ( ret == false ) {
                    chargerInfo.member_card_no = "";
                    resultCd = "2";
                    resultMsg = "해당 아웃렛ID 사용불가";
            ...
            sendGlobLockOpenAck_33(reqId, membercard, outletId, connType,chg_req_amt, resultCd, resultMsg, recvPacket);
        }
        catch (Exception e) {
            LogWrapper.e(TAG, "onGlobLockOpen_33 e:"+e.toString());
        }
    }

    void onEvMembCert_31(KevcsPacket recvPacket) {
        boolean ret = false;
        if (recvPacket.ret == KevcsProtocol.KevcsRet.PKT_NOT.getID()) {
            ret = false;
        }
        else {
            ret = true;
        ...
        if ( kevcsCommListener != null ) kevcsCommListener.onKevcsCommAuthResult(ret);
    }

    void onEVChargeStat_34(KevcsPacket recvPacket) {
        if ( isLocalMode ) {
            isLocalMode = false;
            if (kevcsCommListener != null) kevcsCommListener.onCommLocalMode(false);
        }
    ...
    void onEvMembCard_36(KevcsPacket recvPacket) {
        try {
            if (kevcsCommListener != null)
                kevcsCommListener.onKevcsReadCardNum(recvPacket.getDataAt(0));
        } catch(Exception e) {
            LogWrapper.e(TAG, "onEvMembCard_36 e:"+e.toString());
        }
    }

    void onGlobCardInfo_37(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        try {
            ...
            if (infoNum > 0) {
                kevcsMember.addMembers(listCardNo, listPwd, listAddCl, infoNum);
            }
    ...
    void onGlobPaymStat_61(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        if ( isPaymStatErr ) {
            isPaymStatErr = false;
            if ( kevcsCommListener != null ) kevcsCommListener.onPaymStatErr(false);
        }
    }

    void onGlobChargeAmt_62(KevcsPacket recvPacket) {
        int idx = 0;
        if (recvPacket.ret == PKT_YES.getID()) {
            ...
            try {
                ...
                kevcsPacket.encodeAck(GLOB_CHARGE_AMT_62.getID(), PKT_ACK.getID(), null, recvPacket);
                AddSendServerPacket(kevcsPacket);
                ...
                if ( kevcsCommListener != null ) kevcsCommListener.onPayResult(chargerInfo.payRetInfo);
            } catch (Exception e) {
    ...
    void onGlobPaymAnt_65(KevcsPacket recvPacket) {
        int idx = 0;
    ...
    void onGlobPaymInty_66(KevcsPacket recvPacket) {
        int idx = 0;
    ...
    void onGlobPaymStatSend_67(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        try {
            ...
            kevcsPacket.encodeAck(GLOB_PAYM_STAT_SEND_67.getID(), PKT_YES.getID(), null, recvPacket);
            AddSendServerPacket(kevcsPacket);
    ...
    void onGlobPaymCtl_68(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        try {
            ...
            kevcsPacket.encodeAck(GLOB_PAYM_CTRL_68.getID(), PKT_YES.getID(), null, recvPacket);
            AddSendServerPacket(kevcsPacket);
    ...
    void onGlobFirmUpdate_71(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        if (recvPacket.ret == PKT_DATA.getID()) {
            ...
            if (kevcsCommListener != null) kevcsCommListener.onFinishUpdateDownload();
        }
    }
    void onGlobCpConfig_72(KevcsPacket recvPacket) {
        //여기다 단가 넣기
        int idx = 0;
        costManager.base_paym_amt =Integer.parseInt(recvPacket.getDataAt(0));
        /*chargerInfo.payRxInfo.ucost_aply_tp = recvPacket.getDataAt(idx++);
        chargerInfo.payRxInfo.tax_tp = recvPacket.getDataAt(idx++);
        chargerInfo.payRxInfo.send_port = recvPacket.getDataAt(idx++);
        chargerInfo.payRxInfo.cp_ip = recvPacket.getDataAt(idx++);
        chargerInfo.payRxInfo.paym_ip = recvPacket.getDataAt(idx++);
        chargerInfo.payRxInfo.evcos_ip = recvPacket.getDataAt(idx++);
        chargerInfo.payRxInfo.m2m_fw_ver = recvPacket.getDataAt(idx++);
        chargerInfo.payRxInfo.card_ucost = recvPacket.getDataAt(idx++);*/
        costManager.noMemberCost = Double.parseDouble(recvPacket.getDataAt(7));
    }
    /*void onGlobQRImgDown_73(KevcsPacket recvPacket) {
        int idx = 0;
        ...
        if (recvPacket.ret == PKT_DATA.getID()) {
            ...
            if (kevcsCommListener != null) kevcsCommListener.onQRImageDownFinish(basePath+ "/" + FILE_QR_IMG+"."+strFileExt);
        }
    }*/
    ...
    void onTL3500S_GetConfig(Map<String, String> retVal) {
        try {
            ...
            chargerInfo.payRxInfo.pam_ip = retVal.get("ip").replace(" ", "");;
            chargerInfo.payRxInfo.pam_gw = retVal.get("gateway").replace(" ", "");;
            chargerInfo.payRxInfo.van_ip = retVal.get("van_ip").replace(" ", "");;
            chargerInfo.payRxInfo.van_port = retVal.get("van_port").replace(" ", "");;
            chargerInfo.payRxInfo.paym_stat = "1";
            chargerInfo.payRxInfo.paym_update_stat = "1";
            chargerInfo.payRxInfo.m2m_fw_ver = "0";
            chargerInfo.payRxInfo.paym_tid = retVal.get("mid").replace(" ", "");;
    ...
    void onTL3500S_Event(Map<String, String> retVal, int ch) {
        if ( retVal.get("event") != null ) {
            kevcsCommListener.onKevcsTL3500CardEvent(retVal.get("event"));
    ...
    void onTL3500S_Search(Map<String, String> retVal, int ch) {
        ...
        kevcsCommListener.onKevcsReadCardNum(cnum);
    }
    ...
    void onTL3500S_CancelPay(Map<String, String> retVal, int ch) {
        try {
            ...
            if ( retVal.get("payCode").equals("1")) { // 정상 승인
                ...
                chargerInfo.payRetInfo.charge_amt = chargerInfo.paym_charge_amt;
                chargerInfo.payRetInfo.aprv_amt = chargerInfo.paym_charge_amt;
                chargerInfo.payRetInfo.pay_stat = "05"; // 취소실패
                chargerInfo.payRetInfo.pay_ret_code = retVal.get("retCode");
                chargerInfo.payRetInfo.pay_ret_msg1 = retVal.get("errMsg").replaceAll("\\p{Z}", "").trim();
            }
    ...
    // Test Comm
    //=============================================
    public boolean sendTestCmd(KevcsProtocol.KevcsCmd cmd) {
        boolean ret = true;
        switch (cmd) {
            case GLOB_STAT_PING_10:
                sendGlobStatPing_10();
