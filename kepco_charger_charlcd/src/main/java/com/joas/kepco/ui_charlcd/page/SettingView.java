...
import com.joas.kepco.ui_charlcd.SoundManager;
...
import com.joas.kepcocomm.KepcoCostInfo;
...
public class SettingView extends LinearLayout implements PageActivateListener {
    ...
    void initButtonEvent() {
        ...
        Button  btLastPaymCancel = (Button) findViewById(R.id.btLastPaymCancel);
        btLastPaymCancel.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                //framePaymInty.setVisibility(VISIBLE);
                flowManager.lastPaymCancel();
            }
        });
    ...
    public void onCommTest() {
        ...
        KepcoProtocol.KepcoCmd cmd = KepcoProtocol.KepcoCmd.getValue(strCmd);
        if ( flowManager.getKepcoComm().sendTestCmd(cmd) ) {
    ...
    public void onPaymInitChk() {
        flowManager.getKepcoComm().sendGlobPaymInty_66(false);
    }
    ...
    public void onUpdateUsb() {
        ...
        try {
            File file = new File(updatePath + "/" + KepcoComm.FILE_UPDATE_FIRMWARE_APK);
            if (file.exists()) {
    ...
    void updateStatus() {
        ...
        if ( flowManager.getCpConfig().usePayTerminal ) {
            KepcoChargerInfo info = flowManager.getKepcoComm().getChargerInfo();
        ...
        tvControlBoardVer.setText(flowManager.getKepcoComm().getChargerInfo().board_ver);
        tvControlBoardVer.invalidate();

        KepcoCostInfo costInfo  = flowManager.getKepcoComm().getCostManager().getCurrentCostInfo();

        tvCostUnitInfo.setText(String.format("단가:%.2f원, 버전:%s",flowManager.getKepcoComm().getCurrentCostUnit(),costInfo.version));
