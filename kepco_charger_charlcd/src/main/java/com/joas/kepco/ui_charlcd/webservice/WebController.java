...
class WebController {
    ...
    String sw_update(@RequestParam(name = "file") MultipartFile file, @RequestParam(name = "token") String token) throws IOException {
        ...
        try {
            ...
            RemoteUpdater updater = new RemoteUpdater(KepcoCharLCDUIActivity.getMainActivity(), updatePath, "update.apk");
    ...
    String factoryreset(@PathVariable(name = "token") String token) {
        ...
        try {
            ...
            result.put("result", "success");
            KepcoCharLCDUIActivity.getMainActivity().getFlowManager().doFactoryReset();
        }
        catch(Exception e) {}
    ...
    String get_commlogfiledown(@PathVariable(name = "token") String token, @PathVariable(name = "filename") String filename) {
        ...
        try {
            content = FileUtil.getStringFromFile(Environment.getExternalStorageDirectory() + TypeDefine.KEPCO_COMM_PATH+"/Log/" + filename);
        }
        catch (Exception e) {
