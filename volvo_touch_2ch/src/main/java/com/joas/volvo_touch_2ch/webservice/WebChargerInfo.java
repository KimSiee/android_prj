...
package com.joas.volvo_touch_2ch.webservice;

import android.support.v4.util.CircularArray;
import android.util.ArrayMap;

import com.joas.volvo_touch_2ch.VolvoTouch2chUIActivity;
import com.joas.volvo_touch_2ch.CPConfig;
import com.joas.volvo_touch_2ch.MultiChannelUIManager;
import com.joas.volvo_touch_2ch.TypeDefine;
...
public class WebChargerInfo {
    ...
    public static JSONObject getVersions() {
        UIFlowManager flowManager = VolvoTouch2chUIActivity.getMainActivity().getMultiChannelUIManager().getUIFlowManager(0);
    ...
    public static JSONObject getChargerStat() {
        MultiChannelUIManager manager = VolvoTouch2chUIActivity.getMainActivity().getMultiChannelUIManager();
        ...
        try {
            ...
            json.put("comm", VolvoTouch2chUIActivity.getMainActivity().getIsCommConnected() ? "Connected" : "Disconnected");
...
//        manager.getOcppSessionManager().getOcppConfiguration().setConfigration(key, val);
    ...
    public static void setChargerSetting(JSONObject json) {
        ...
        try {
            ...
            cpConfig.saveConfig(VolvoTouch2chUIActivity.getMainActivity().getApplicationContext());
    ...
    public static JSONArray getRecentSysLog() {
        CircularArray<LogWrapperMsg> queue = VolvoTouch2chUIActivity.getMainActivity().getMultiChannelUIManager().getJoasDebugMsgView().getPacketQueue();
        int cnt = queue.size();
        JSONArray array = new JSONArray();
        try {
            synchronized (queue) {
    ...
    public static JSONArray getRecentCommLog() {
        CircularArray<JoasCommMonitorView.CommMonitorMsg> queue = VolvoTouch2chUIActivity.getMainActivity().getMultiChannelUIManager().getCommMonitorView().getPacketQueue();
        int cnt = queue.size();
        JSONArray array = new JSONArray();
        try {
            synchronized (queue) {
