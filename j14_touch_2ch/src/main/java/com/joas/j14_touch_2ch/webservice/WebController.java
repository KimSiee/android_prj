...
class WebController {
    ...
    String sw_update(@RequestParam(name = "file") MultipartFile file, @RequestParam(name = "token") String token) throws IOException {
        ...
        try {
            ...
            RemoteUpdater updater = new RemoteUpdater(J14Touch2chUIActivity.getMainActivity(), updatePath, "update.apk");
    ...
    String factoryreset(@PathVariable(name = "token") String token) {
        ...
        try {
            ...
            result.put("result", "success");
            J14Touch2chUIActivity.getMainActivity().getMultiChannelUIManager().doFactoryReset();
        }
        catch(Exception e) {}
