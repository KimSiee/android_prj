...
public class WebChargerInfo {
    ...
    public static JSONObject getChargerStat() {
        ...
        try {
            ...
            json.put("comm", KevcsCharLCDUIActivity.getMainActivity().getFlowManager().getKevcsComm().getConnectStatus() ? "Connected" : "Disconnected");
        }
        catch (Exception e) {}
        return json;
    }
    ...
    public static void setChargerSetting(JSONObject json) {
        ...
        try {
            ...
            cpConfig.saveConfig(KevcsCharLCDUIActivity.getMainActivity().getApplicationContext());
    ...
    public static JSONArray getRecentSysLog() {
        CircularArray<LogWrapperMsg> queue = KevcsCharLCDUIActivity.getMainActivity().getPageManger().getJoasDebugMsgView().getPacketQueue();
        int cnt = queue.size();
    ...
    public static JSONArray getRecentCommLog() {
        CircularArray<UIFlowManager.CommMonitorMsg> queue = KevcsCharLCDUIActivity.getMainActivity().getFlowManager().getCommLogPacketQueue();
        int cnt = queue.size();
        JSONArray array = new JSONArray();
        try {
            synchronized (queue) {
    ...
    public static JSONArray getCommLogFiles() {
        JSONArray array = new JSONArray();
        try {
            File file = new File(Environment.getExternalStorageDirectory(), TypeDefine.KEVCS_COMM_PATH+"/Log");
