...
package com.joas.ocppui_tardis.webservice;
...
import com.joas.ocppui_tardis.CPConfig;
import com.joas.ocppui_tardis.OCPPUITardisActivity;
import com.joas.ocppui_tardis.TypeDefine;
import com.joas.ocppui_tardis.UIFlowManager;
import com.joas.ocppui_tardis.page.JoasCommMonitorView;
...
public class WebChargerInfo {
    ...
    public static JSONObject getVersions() {
        UIFlowManager flowManager = OCPPUITardisActivity.getMainActivity().getFlowManager();
    ...
    public static JSONObject getChargerStat() {
        ...
        try {
            ...
            json.put("comm", OCPPUITardisActivity.getMainActivity().getIsCommConnected() ? "Connected" : "Disconnected");
    ...
    public static void setChargerSetting(JSONObject json) {
        ...
        try {
            ...
            cpConfig.saveConfig(OCPPUITardisActivity.getMainActivity().getApplicationContext());
    ...
    public static JSONArray getRecentSysLog() {
        CircularArray<LogWrapperMsg> queue = OCPPUITardisActivity.getMainActivity().getPageManger().getJoasDebugMsgView().getPacketQueue();
        int cnt = queue.size();
        JSONArray array = new JSONArray();
        try {
            synchronized (queue) {
    ...
    public static JSONArray getRecentCommLog() {
        CircularArray<JoasCommMonitorView.OCPPMonitorMsg> queue = OCPPUITardisActivity.getMainActivity().getPageManger().getCommMonitorView().getPacketQueue();
    ...
    public static JSONArray getAuthCacheList() {
        Map<String, IdTagInfo> list = OCPPUITardisActivity.getMainActivity().getFlowManager().getOcppSessionManager().getOcppStack().getAuthorizeCache().getAllCacheIdTag();
        JSONArray array = new JSONArray();
        try {
            String k;
            IdTagInfo v;
    ...
    public static JSONArray getLocalAuthList() {
        Map<String, IdTagInfo> list = OCPPUITardisActivity.getMainActivity().getFlowManager().getOcppSessionManager().getOcppStack().getLocalAuthList().getAllLocalAuthList();
        JSONArray array = new JSONArray();
        try {
            String k;
            IdTagInfo v;
