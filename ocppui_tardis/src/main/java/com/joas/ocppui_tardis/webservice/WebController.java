...
class WebController {
    ...
    String sw_update(@RequestParam(name = "file") MultipartFile file, @RequestParam(name = "token") String token) throws IOException {
        ...
        try {
            ...
            RemoteUpdater updater = new RemoteUpdater(OCPPUITardisActivity.getMainActivity(), updatePath, "update.apk");
    ...
    String factoryreset(@PathVariable(name = "token") String token) {
        ...
        try {
            ...
            result.put("result", "success");
            OCPPUITardisActivity.getMainActivity().getFlowManager().doFactoryReset();
        }
        catch(Exception e) {}
