 ...
 * Written by Sungchul Choi <scchoi@joas.co.kr>, 19. 10. 31 오후 2:53
 *
 */
...
class WebController {
    ...
    String sw_update(@RequestParam(name = "file") MultipartFile file, @RequestParam(name = "token") String token) throws IOException {
        ...
        try {
            ...
            RemoteUpdater updater = new RemoteUpdater(OCPPUIActivity.getMainActivity(), updatePath, "update.apk");
    ...
    String factoryreset(@PathVariable(name = "token") String token) {
        ...
        try {
            ...
            result.put("result", "success");
            OCPPUIActivity.getMainActivity().getFlowManager().doFactoryReset();
        }
        catch(Exception e) {}
