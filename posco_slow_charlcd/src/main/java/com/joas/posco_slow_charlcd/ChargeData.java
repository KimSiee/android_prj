/*
 * Copyright (C) 2017 JoongAng Control, Inc - All Rights Reserved
 * Unauthorized copying of this file, via any medium is strictly prohibited
 * Proprietary and confidential
 * Written by Sungchul Choi <scchoi@joas.co.kr>, 21. 1. 13 오후 5:19
 *
 */

package com.joas.posco_slow_charlcd;

import com.joas.ocpp.msg.StatusNotification;

import java.util.Date;

public class ChargeData {
    public int dspChannel = TypeDefine.DEFAULT_CHANNEL;

    /**
     * 커넥터 종류, UI에서 사용자 선택에 의해서 바뀜
     */
    public TypeDefine.ConnectorType connectorType = TypeDefine.ConnectorType.AC3;

    public int chargerType = TypeDefine.CP_TYPE_SLOW_AC;

    /**
     * UI에서 인증 타임아웃 시간, 기본 60초
     */
    public int authTimeout = TypeDefine.DEFAULT_AUTH_TIMEOUT;
    public int connectorWaitTimeout = TypeDefine.DEFAULT_CONNECTOR_WAIT_TIMEOUT ;
    public int connectCarTimeout = TypeDefine.DEFAULT_CONNECT_CAR_TIMEOUT ;
    public int authnumInputTimeout = TypeDefine.NOMEM_INPUT_AUTH_NUM_TIMEOUT;


    // 충전진행시 필요로 하는 공유 변수들(충전량, 시간등)

    public long measureWh  = 0;
    public Date chargeStartTime = new Date();
    public Date chargeEndTime = new Date();
    public long chargingTime = 0;
    public long chargingRemainTime = 0;     //add by si. - 20200528 : 차지비 설정시간에 따른 남은시간 추가
    public double chargingCost = 0;
    public double chargingUnitCost = TypeDefine.DEFAULT_UNIT_COST;     //단가
    public int soc = 0;
    public int remainTime = 0; // 남은시간
    public int partialCancelType = 0;       //부분취소 : 5, 무카드취소 : 4

    /**
     * 메시지 박스 타이틀
     */
    public String messageBoxTitle = "";

    /**
     * 메시지 박스 내용(멀티라인)
     */
    public String messageBoxContent = "";

    /**
     * 메시지 박스 시간(초)
     */
    public int messageBoxTimeout = TypeDefine.MESSAGEBOX_TIMEOUT_SHORT;

    public String faultBoxContent = "";

    /**
     *  회원,비회원 구분
     */
    public TypeDefine.AuthType authType = TypeDefine.AuthType.NONE;

    //선결제 진행 유/무 구분 플래그
    public boolean firstpayFlag = false;
    //실결제 진행 유/무 구분 플래그
    public boolean realpayflag = false;

    public boolean is9511 = false;

    public boolean isdspCommError = false;
    public boolean ismeterCommError = false;

    public String meter_version = "";


    //ocpp관련
    /**
     * 동시에 사용 가능한 총 커넥처(채널) 수
     */
    public int ocppConnectorCnt = TypeDefine.OCPP_CONNECTOR_CNT_1CH_3MODE;

    /**
     * 현재 커넥터 번호(현재 채널), 1개인 경우에는 항상 1이 됨
     */
    public int curConnectorId = TypeDefine.OCPP_DEFAULT_CONNECTOR_ID ;

    // 현재 커넥터에 대한 상태값
    StatusNotification.Status ocppStatus = StatusNotification.Status.AVAILABLE;
    StatusNotification.ErrorCode ocppStatusError = StatusNotification.ErrorCode.NO_ERROR;
}
