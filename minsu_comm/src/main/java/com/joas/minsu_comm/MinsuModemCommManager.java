 ...
 * Written by Sungchul Choi <scchoi@joas.co.kr>, 20. 6. 3 오후 3:09
 *
 */
...
public class MinsuModemCommManager extends Thread{

    public static final int RECV_BUFF_MAX = 1024;
//    public static final int INFO_REQ_PERIOD  = 60;        //1분에 한번씩 요청(test)
    public static final int INFO_REQ_PERIOD  = 1800;        //30분에 한번씩 요청(test)
    String modemIP = "";
    int modemPort;
    MinsuModemCommManagerListener listener;
    ...
    public MinsuModemCommManager(Context context, String ipAddress, int port, MinsuModemCommManagerListener minsuModemCommManagerListener) {
        modemIP = ipAddress;
        modemPort = port;
        listener = minsuModemCommManagerListener;

    }


    public void run() {
        int periodCnt = INFO_REQ_PERIOD;
        while (!isInterrupted()) {
            try {
                periodCnt++;
                ...
                LogWrapper.e("MinsuModemComm",e.toString());
            }
