...
public class MinsuPacket {
     ...
     */
    public static int fillBasicField(byte[] vdData, MinsuChargerInfo info) {
        int idx = 0;
    ...
    public static int fillChargingStatus(byte[] vdData, int idx, MinsuChargerInfo info) {
        ...
        int remainTime = info.remainTime;       //dsp로부터 분단위로 받음
        try {
...
//            arr_remtime[1] = (byte) ((remainTime % 3600) / 60);
            ...
            //분단위에서 시분으로 변환
            arr_remtime[0] = (byte) (remainTime / 60);
            arr_remtime[1] = (byte) (remainTime % 60);
...
//        long remainTime = info.remainTime/60 + info.remainTime%60;
...
//        vdData[idx++] = hexRemainTime[1];
        ...
        //커플러 온도(4)
        int connectortemp = info.connector_temp*10;
        ByteUtil.intToByteArray(connectortemp, vdData, idx);
        idx += 4;


        //충전기 시작 SOC(2)
        vdData[idx++] = info.start_socBatt;
        vdData[idx++] = 0;

        //충전시간(2)
        int chargingTime = info.chargingTime;
        try {
            byte[] arr_cgtime = new byte[2];
            arr_cgtime[0] = (byte) (chargingTime / 3600);
            arr_cgtime[1] = (byte) ((chargingTime % 3600) / 60);

            ByteUtil.GetDectoHexBCDFormat(arr_cgtime);
            safeArrayCopy(arr_cgtime, 0, vdData, idx, 2);
    ...
    public static int fillInstallationInfo(byte[] vdData, int idx, MinsuChargerInfo info) {
    ...
    public static class BasicResponse {
        ...
        public byte[] encode(MinsuChargerInfo info) {
    ...
    public static class BasicResponseCode {
        ...
        public byte[] encode(MinsuChargerInfo info, int ackCode, int reasonCode) {
            byte[] vdData = new byte[VD_DATA_SIZE];
    ...
    */
    ...
    public static class CostInfoDown_G1_1j {
        ...
        public int[] costunit = new int[24];
        ...
        public void decode(byte[] vdData) {
            ...
            //버전정보
            version = ByteUtil.byteArrayToHexString(vdData, idx, 8);
            idx += 8;
            ...
            for (int i = 0; i < 24; i++) {
                //시간별 단가정보 저장
                costunit[i] = ByteUtil.makeInt(vdData, idx, true);
                idx += 4;
            }
        }
    }
...
//        }
    ...
    public static class AlarmInfo_c1 {
        ...
        public byte[] encode(MinsuChargerInfo info, int alarmCode, byte alarmStatus) {
            byte[] vdData = new byte[VD_DATA_SIZE];
    ...
    public static class FinishCharging_f1 {
        ...
        public byte[] encode(MinsuChargerInfo info) {
            ...
            //충전기 종료 SOC
            vdData[idx++] = info.finish_socBatt;
            vdData[idx++] = 0;
...
//            idx += 7;
    ...
    public static class NonTransPacket_i1 {
        public byte[] encode(MinsuChargerInfo info, int dataIndex, byte[] rawData) {
    ...
    public static class ChargerFirmwareReq_m1 {
        ...
        public byte[] encode(MinsuChargerInfo info, byte dest1, byte dest2) {
            byte[] vdData = new byte[VD_DATA_SIZE];
            int idx = fillBasicField(vdData, info);
