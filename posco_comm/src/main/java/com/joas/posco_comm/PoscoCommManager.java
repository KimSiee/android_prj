...
public class PoscoCommManager extends EvCommManager {
    ...
    PoscoCommManagerListener poscoCommManagerListener = null;

    PoscoMemberManager memberManager;

    PoscoChargerInfo chargerInfo;
    ...
    public PoscoCommManager(Context context, String svrip, int svrport, String sid, String cid, int type, PoscoChargerInfo info, String filePath, int timeout)  {
    ...
    public void setPoscoCommManagerListener(PoscoCommManagerListener listener) { poscoCommManagerListener = listener; }
    ...
    void onRecvChangeModeReq_C1(EvPacket packet) {
        try {
            ...
            if ( poscoCommManagerListener != null ) poscoCommManagerListener.onChangeChargerMode(request.mode);
        }
        catch (Exception e) {
    ...
    void onRecvCostDownReqResp_G1_1j(EvPacket packet, boolean isRequestPacket) {
        try {
            ...
            if ( poscoCommManagerListener != null ) poscoCommManagerListener.onRecvCostInfo(chargerInfo);
                    ...
                    if (infoDownBlockIdx < request.blockIdx) {
                        ...
                        if (request.div == (byte) 0xFF) {
                            if (request.dest2 == (byte) 0x01)
                                poscoCommManagerListener.onFirmwareDownCompleted(infoDownData);
    ...
    void onChargerInfoChangeReq_N1(EvPacket packet) {
        try {
            ...
            if ( poscoCommManagerListener != null ) poscoCommManagerListener.onChargerInfoChangeReq(request.kind, request.changeVal);
        }
        catch (Exception e) {
    ...
    void onRecvInfDown_K2(EvPacket packet){
        ...
        if(!request.remoteURL.equals("")){
            ...
            if ( poscoCommManagerListener != null ) ret = poscoCommManagerListener.onReqInfoDownByHttp(request.dest1,request.dest2,request.remoteURL);
...
//                chargerInfo.isRemoteCharge = false;       //edit by si. 211006 : 카드태깅,원격 UID가 혼용되어 올라가는 부분 수정
            ...
            if ( poscoCommManagerListener != null ) ret = poscoCommManagerListener.onRemoteStartStop(request.chargingCmd);
    ...
    void SetFaultStatus(int code, byte status) {
        ...
        if (code == AlarmCode.ERR_CODE_11 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFFFFFFF7FFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000000000080000L;

        //전력량계 통신이상
        if (code == AlarmCode.ERR_CODE_12 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFFFFDFFFFFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000000020000000L;

        //비상스위치 모니터링
        if(code == AlarmCode.EMERGENCY && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFFFFFFFFBFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000000000004000L;

        //융착모니터링(릴레이,MC 융착)
        if(code == AlarmCode.ERR_CODE_9 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFFDFFFFFFFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000002000000000L;

        //rcd trip 모니터링
        if(code == AlarmCode.ERR_CODE_7 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFF7FFFFFFFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000008000000000L;

        //접지오류(FG불량)
        if(code == AlarmCode.ERR_CODE_3 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFEFFFFFFFFFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000100000000000L;

        //입력저전압
        if(code == AlarmCode.ERR_CODE_5 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFFFFFBFFFFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000000004000000L;

        //입력과전압
        if(code == AlarmCode.ERR_CODE_6 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFFFFFFFFFEFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000000000000100L;

        //출력과전류
        if(code == AlarmCode.ERR_CODE_8 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFFFFFFFFF7FFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000000000000800L;

        //cp레벨상태 오류(차량통신이상)
        if(code == AlarmCode.ERR_CODE_10 && status == AlarmCode.STATE_OCCUR)
            chargerInfo.cpStatus = chargerInfo.cpStatus & 0xFFFFFBFFFFFFFFFFL;
        else chargerInfo.cpStatus = chargerInfo.cpStatus | 0x0000040000000000L;
    ...
    void onRecvAuthResp_1a(EvPacket packet) {
        try {
            ...
            poscoCommManagerListener.onAuthResultEvent(ack.authResult, (int)ack.memberKindCode, ack.costUnit);
    ...
    void onRecvStartChargingResp_1d(EvPacket packet) {
        try {
            ...
            if(poscoCommManagerListener!=null) poscoCommManagerListener.onRecvStartChargingResp(ack.retCode,ack.reasonCode);
    ...
    void onRecvFinishChargingResp_1f(EvPacket packet) {
        try {
            ...
            if ( poscoCommManagerListener != null ) poscoCommManagerListener.onRecvFinishChargingResp(ack.chargingCost);
        }
        catch (Exception e) {
    ...
    void onRecvVersionResp_1r(EvPacket packet) {
        try {
            ...
            if ( poscoCommManagerListener != null ) poscoCommManagerListener.onRecvVersionResp(ack.verinfo);
        }
        catch (Exception e) {
            LogWrapper.e(TAG, "onRecvFinishChargingResp_1f:" + e.toString()+", "+ Log.getStackTraceString(e));
        }
    ...
    public void onCommConnected() {
        ...
        if (poscoCommManagerListener != null) poscoCommManagerListener.onCommConnected();
    }

    @Override
    public void onCommDisconnected() {
        if (poscoCommManagerListener != null) poscoCommManagerListener.onCommDisconnected();
    }
